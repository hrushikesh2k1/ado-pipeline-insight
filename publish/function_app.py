...
def _extract_organization(payload: dict, resource: dict) -> str | None:
    ...
    if explicit:
        return explicit
    url_candidates = [resource.get("url"), resource.get("_links", {}).get("web", {}).get("href")]
    for value in url_candidates:
        ...
        if host.endswith(".visualstudio.com"):
            return host.split(".")[0]
    return payload.get("resourceContainers", {}).get("account", {}).get("name")
...
def get_recommendations(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        if repository.count_runs(pipeline_id) < settings.min_history_runs:
            return func.HttpResponse(json.dumps({"message": f"Not enough run history yet for this pipeline - need at least {settings.min_history_runs} runs."}), status_code=200, mimetype="application/json")
